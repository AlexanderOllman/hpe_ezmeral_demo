...
class smileProcessing:

    def __init__(self):
        self.stopped = False
        self.smile_rects = None
        self.rejectLevels = None
        self.levelWeights = None
        self.roi = None
        self.smile_cascade = cv2.CascadeClassifier('haarcascade_smile.xml')

    def start(self):
        Thread(target=self.getSmiles, args=()).start()
        return self

    def getSmiles(self):
        while not self.stopped:
            self.smile_rects, self.rejectLevels, self.levelWeights = self.smile_cascade.detectMultiScale3(self.roi, 2.5, 20, outputRejectLevels=True)
...
def gen_frames():  
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            roi = faceRecognition.gray[y:y + h, x:x + w]
            ...
            smileRecognition.roi = roi
            levelWeights = smileRecognition.levelWeights
