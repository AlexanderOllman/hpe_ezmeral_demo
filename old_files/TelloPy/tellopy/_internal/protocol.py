...
class Packet(object):
    def __init__(self, cmd, pkt_type=0x68, payload=b''): # If cmd is passed, packet is a byte array.
        if isinstance(cmd, str):
            ...
            self.buf = bytearray()
            self.buf[:] = cmd
        else:                                            # Else a new empty packet is created
            self.buf = bytearray([
                START_OF_PACKET,
                0, 0,
                0,
    ...
    def fixup(self, seq_num=0):     # Add a CRC at the end of the packet is the packet is correctly formated
        buf = self.get_buffer()
